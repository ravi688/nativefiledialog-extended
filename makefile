...
#-------------------------------------------
PROJECT_NAME = nfd
STATIC_LIB_NAME = nfd.a
DYNAMIC_LIB_NAME = nfd.dll
...
ifeq ($(PLATFORM),Windows)
	EXCLUDE_SOURCES+=source/nfd_gtk.cpp source/nfd_portal.cpp
	EXTERNAL_LIBRARIES+=-lole32 -luuid -lshell32
endif
ifeq ($(PLATFORM),Linux)
	EXCLUDE_SOURCES+=source/nfd_win.cpp source/nfd_portal.cpp
	EXTERNAL_INCLUDES+=$(shell pkg-config gtk+-3.0 --cflags)
	EXTERNAL_LIBRARIES+=$(shell pkg-config gtk+-3.0 --libs)
endif
